...
# Lab 14 — Flow Orchestration (Оркестрація потоків)
- **Юзкейс**: Stateful потік обробки документів (Ingest -> Route -> Execute -> Validate -> Export).
- **Типи вхідних даних**: Фінансові перекази, посилання на документи, шумний текст, порожні входи.
- **Маршрути (Routes)**: `finance_extraction`, `document_signal_schema`, `manual_review_route`.
- **Ресурси знань**: JSON-схеми, регулярні вирази для fallback.
- **Метрики**: 90% Flow completion rate, ~78% успішного відновлення через fallback.
- **Покращена керованість**: Збереження стану дозволяє проводити точний аудит кожного етапу та контрольоване відновлення через агентів ремонту або правила.